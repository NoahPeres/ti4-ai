...
class TestStatusPhaseScoringLimits:
    ...
    def test_can_score_one_public_objective_per_status_phase(self):
        ...
        with pytest.raises(ValueError, match="Already scored a public objective during this status phase"):
    ...
    def test_can_score_one_secret_objective_per_status_phase(self):
        ...
        secret_obj1 = Objective("sec1", "Secret 1", "First secret", 1, False, GamePhase.STATUS)
        secret_obj2 = Objective("sec2", "Secret 2", "Second secret", 1, False, GamePhase.STATUS)
        
        # Assign secret objectives to player first
        state_with_secrets = game_state.assign_secret_objective("player1", secret_obj1)
        state_with_secrets = state_with_secrets.assign_secret_objective("player1", secret_obj2)
        
        # Score first secret objective - should succeed
        state1 = state_with_secrets.score_objective("player1", secret_obj1, GamePhase.STATUS)
        ...
        with pytest.raises(ValueError, match="Already scored a secret objective during this status phase"):
    ...
    def test_can_score_one_public_and_one_secret_per_status_phase(self):
        ...
        public_obj = Objective("pub", "Public", "Public objective", 1, True, GamePhase.STATUS)
        secret_obj = Objective("sec", "Secret", "Secret objective", 1, False, GamePhase.STATUS)
        ...
        state1 = state_with_secret.score_objective("player1", public_obj, GamePhase.STATUS)
        assert state1.get_victory_points("player1") == 1
...
class TestCombatScoringLimits:
    ...
    def test_can_score_one_objective_per_combat(self):
        ...
        action_obj1 = Objective("act1", "Action 1", "First action", 1, True, GamePhase.ACTION)
        action_obj2 = Objective("act2", "Action 2", "Second action", 1, True, GamePhase.ACTION)
        
        combat_id = "combat_123"
        
        # Score first objective during combat - should succeed
        state1 = game_state.score_objective_during_combat("player1", action_obj1, combat_id)
        assert state1.is_objective_completed("player1", action_obj1)
        assert state1.get_victory_points("player1") == 1
        
        # Try to score second objective in same combat - should fail
        with pytest.raises(ValueError, match="Already scored an objective during combat"):
    ...
    def test_different_combats_have_separate_limits(self):
        ...
        state1 = game_state.score_objective_during_combat("player1", action_obj1, combat_id1)
        
        # Score objective in second combat - should succeed
        state2 = state1.score_objective_during_combat("player1", action_obj2, combat_id2)
    ...
    def test_combat_scoring_requires_action_phase_objective(self):
        ...
        status_obj = Objective("stat", "Status", "Status objective", 1, True, GamePhase.STATUS)
        combat_id = "combat_123"
        
        # Try to score status phase objective during combat - should fail
        with pytest.raises(ValueError, match="Cannot score status phase objective during combat"):
...
class TestStatusPhaseStepExecution:
    ...
    def test_execute_status_phase_step_1_multiple_objectives(self):
        ...
        final_state = state_with_secret.execute_status_phase_step_1_score_objectives("player1", objectives)
    ...
    def test_execute_status_phase_step_1_respects_limits(self):
        ...
        with pytest.raises(ValueError, match="Already scored a public objective during this status phase"):
            game_state.execute_status_phase_step_1_score_objectives("player1", objectives)
...
class TestObjectiveScoringEdgeCases:
    ...
    def test_non_status_phase_objectives_ignore_status_limits(self):
        ...
        action_obj = Objective("act", "Action", "Action objective", 1, True, GamePhase.ACTION)
        status_obj = Objective("stat", "Status", "Status objective", 1, True, GamePhase.STATUS)
