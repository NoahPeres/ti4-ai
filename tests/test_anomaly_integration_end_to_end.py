...
class TestAnomalySystemIntegration:
    ...
    def test_anomaly_effects_summary_integration(self):
        ...
        # Test nebula effects
        nebula_effects = self.anomaly_manager.get_anomaly_effects_summary(
            self.nebula_system
        )
        assert (
        ...
        gravity_effects = self.anomaly_manager.get_anomaly_effects_summary(
            self.gravity_rift_system
        )
        assert (
            gravity_effects["blocks_movement"] is False
        )  # Gravity rift doesn't block movement
        ...
        normal_effects = self.anomaly_manager.get_anomaly_effects_summary(
            self.normal_system
        )
    ...
    def test_movement_blocking_detection(self):
        ...
        assert (
            self.anomaly_manager.is_system_blocking_movement(self.asteroid_system)
            is True
        )
        assert (
            self.anomaly_manager.is_system_blocking_movement(self.supernova_system)
            is True
        )
        assert (
        ...
        assert (
            self.anomaly_manager.is_system_blocking_movement(self.gravity_rift_system)
            is False
        )
        assert (
            self.anomaly_manager.is_system_blocking_movement(self.normal_system)
            is False
        )
...
class TestAnomalySystemCompatibility:
    ...
    def test_existing_system_tile_compatibility(self):
        ...
        # Create anomaly system separately (SystemTile and System are different concepts)
        anomaly_system = self.anomaly_manager.create_anomaly_system(
            "test_system", [AnomalyType.NEBULA]
        )

        # Verify both tile and system work as expected
        assert anomaly_tile.is_anomaly()  # Tile level
        assert anomaly_system.is_anomaly()  # System level
    ...
    def test_dynamic_anomaly_assignment_compatibility(self):
        ...
        # Remove one anomaly type
        self.anomaly_manager.remove_anomaly_from_system(
            anomaly_system, AnomalyType.GRAVITY_RIFT
        )
...
class TestAnomalyPerformanceAndValidation:
    ...
    def test_anomaly_validation_edge_cases(self):
        ...
        system = self.anomaly_manager.create_anomaly_system("empty_system", [])
    ...
    def test_memory_usage_with_complex_anomaly_systems(self):
        ...
        for _ in range(100):
            ...
            self.anomaly_manager.get_combat_modifiers(complex_system, True)
...
class TestAnomalyErrorHandlingIntegration:
    ...
    def test_system_state_consistency_validation(self):
        ...
        # validate_system_state returns None if validation passes (no exception raised)
        result = self.anomaly_manager.validate_system_state(system)
        assert result is None  # No exception means validation passed
        assert not system.is_anomaly()  # System should not be anomaly with empty list
