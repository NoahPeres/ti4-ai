...
class TestSystemWormholeValidation:
    ...
    def test_add_valid_wormhole_types(self):
        ...
        for wormhole_type in valid_types:
            ...
            assert system.has_wormhole(wormhole_type), f"Should have {wormhole_type} wormhole"
    ...
    def test_add_invalid_wormhole_type_raises_error(self):
        ...
        for invalid_type in invalid_types:
            with pytest.raises(ValueError, match=f"Invalid wormhole type: {invalid_type}"):
    ...
    def test_has_wormhole_returns_false_for_nonexistent(self):
        ...
        assert not system.has_wormhole("alpha"), "Should not have alpha wormhole initially"
        assert not system.has_wormhole("beta"), "Should not have beta wormhole initially"
        assert not system.has_wormhole("invalid"), "Should not have invalid wormhole type"
    ...
    def test_get_wormhole_types_returns_copy(self):
        ...
        assert len(system.get_wormhole_types()) == original_length, "Original system should be unchanged"
    ...
    def test_remove_wormhole_success(self):
        ...
        assert not system.has_wormhole("alpha"), "Should not have alpha wormhole after removal"
    ...
    def test_wormhole_operations_integration(self):
        ...
        assert len(system.get_wormhole_types()) == 1, "Should have 1 wormhole after removal"
        assert not system.has_wormhole("alpha"), "Should not have alpha wormhole after removal"
