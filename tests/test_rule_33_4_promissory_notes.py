...
class TestRule334PromissoryNoteElimination:
    ...
    def test_promissory_notes_returned_on_elimination(self) -> None:
        ...
        note_from_player2 = PromissoryNote(PromissoryNoteType.TRADE_AGREEMENT, "player2")
        note_from_player3 = PromissoryNote(PromissoryNoteType.SUPPORT_FOR_THE_THRONE, "player3")
        note_from_player1 = PromissoryNote(PromissoryNoteType.CEASEFIRE, "player1")
        
        # Add notes to player1's hand
        game_state.promissory_note_manager.add_note_to_hand(note_from_player2, "player1")
        game_state.promissory_note_manager.add_note_to_hand(note_from_player3, "player1")
        game_state.promissory_note_manager.add_note_to_hand(note_from_player1, "player2")
        ...
        player2_hand_after = new_game_state.promissory_note_manager.get_player_hand("player2")
        assert note_from_player1 not in player2_hand_after
        
        # Verify player1's hand is empty (player eliminated)
        player1_hand_after = new_game_state.promissory_note_manager.get_player_hand("player1")
    ...
    def test_promissory_notes_removed_from_available_pool(self) -> None:
        ...
        assert game_state.promissory_note_manager.is_note_available_for_transaction(note) is True
        
        # Eliminate player1
        new_game_state = game_state.eliminate_player("player1")
        
        # Verify note is no longer available
        assert new_game_state.promissory_note_manager.is_note_available_for_transaction(note) is False
    ...
    def test_elimination_with_no_promissory_notes(self) -> None:
        ...
        assert len(new_game_state.promissory_note_manager.get_player_hand("player1")) == 0
        assert len(new_game_state.promissory_note_manager.get_player_hand("player2")) == 0