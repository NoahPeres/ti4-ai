...
class TestRule33_1EliminationConditions:
    ...
    def test_player_not_eliminated_with_production_units(self):
        ...
        # Assert
        assert not is_eliminated, "Player with production units should not be eliminated"
    ...
    def test_player_not_eliminated_with_controlled_planet(self):
        ...
        _, game_state = game_state.gain_planet_control("player1", planet)
        
        # Act
        is_eliminated = game_state.should_eliminate_player("player1")
        
        # Assert
        assert not is_eliminated, "Player who controls a planet should not be eliminated"
    ...
    def test_player_eliminated_with_all_three_conditions(self):
        ...
        # Assert
        assert is_eliminated, "Player with no ground forces, no production units, and no controlled planets should be eliminated"
    ...
    def test_player_not_eliminated_with_only_two_conditions(self):
        ...
        # Assert
        assert not is_eliminated, "Player should not be eliminated when only two of three conditions are met"
    ...
    def test_elimination_check_for_nonexistent_player(self):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Player nonexistent_player does not exist"):
