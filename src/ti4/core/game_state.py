...
class GameState:
    ...
    status_phase_scoring: dict[str, dict[str, int]] = field(default_factory=dict, hash=False)  # player_id -> {public: count, secret: count}
    combat_scoring: dict[str, list[str]] = field(default_factory=dict, hash=False)  # combat_id -> [objective_ids]
    
    # Secret objective system (Rule 61.19-61.20)
    player_secret_objectives: dict[str, list[Objective]] = field(default_factory=dict, hash=False)  # player_id -> [secret_objectives]
    secret_objective_deck: list[Objective] = field(default_factory=list, hash=False)  # Deck of unassigned secret objectives
    player_influence: dict[str, int] = field(default_factory=dict, hash=False)  # player_id -> influence_amount
    ...
    def score_objective_during_combat(self, player_id: str, objective: "Objective", combat_id: str) -> "GameState":
        ...
        if combat_id in self.combat_scoring and len(self.combat_scoring[combat_id]) >= 1:
        ...
        if objective.scoring_phase != GamePhase.ACTION:
            raise ValueError(f"Cannot score {objective.scoring_phase.value} phase objective during combat")
    ...
    def execute_status_phase_step_1_score_objectives(self, player_id: str, objectives: list["Objective"]) -> "GameState":
        ...
        for objective in objectives:
            current_state = current_state.score_objective(player_id, objective, GamePhase.STATUS)
    ...
    def _validate_status_phase_scoring_limits(self, player_id: str, objective: "Objective") -> None:
        """Validate status phase scoring limits (Rule 61.6)."""
        player_scoring = self.status_phase_scoring.get(player_id, {"public": 0, "secret": 0})
        
        if objective.is_public and player_scoring.get("public", 0) >= 1:
            raise ValueError(f"Already scored a public objective during this status phase for player '{player_id}'")
        
        if not objective.is_public and player_scoring.get("secret", 0) >= 1:
            raise ValueError(f"Already scored a secret objective during this status phase for player '{player_id}'")

    def _update_status_phase_scoring(self, player_id: str, objective: "Objective", current_phase: GamePhase) -> dict[str, dict[str, int]]:
    ...
    def assign_secret_objective(self, player_id: str, objective: "Objective") -> "GameState":
        ...
        if any(obj.id == objective.id for obj in player_secrets):
            raise ValueError(f"Player {player_id} already has secret objective {objective.id}")
        
        # Check 3 secret objective limit (Rule 61.20)
        if len(player_secrets) >= 3:
            raise ValueError(f"Player {player_id} already has maximum of 3 secret objectives")
        ...
        return self._create_new_state(player_secret_objectives=new_player_secret_objectives)
    ...
    def score_objective(self, player_id: str, objective: "Objective", current_phase: GamePhase) -> "GameState":
        """
    ...
    def _validate_objective_scoring(self, player_id: str, objective: "Objective", current_phase: GamePhase) -> None:
    ...
    def _validate_secret_objective_ownership(self, player_id: str, objective: "Objective") -> None:
        ...
        if not any(obj.id == objective.id for obj in player_secrets):
            raise ValueError(f"Cannot score secret objective {objective.id} - not owned by player {player_id}")
    
    def _validate_objective_timing(self, objective: "Objective", current_phase: GamePhase) -> None:
    ...
    def _validate_objective_not_already_scored(self, player_id: str, objective: "Objective") -> None:
        """Validate that the objective hasn't already been scored by this player."""
        if self.is_objective_completed(player_id, objective):
            raise ValueError(f"Objective '{objective.id}' already scored by player '{player_id}'")
    
    def _execute_objective_scoring(self, player_id: str, objective: "Objective", current_phase: GamePhase) -> "GameState":
        """Execute the objective scoring with all state updates."""
        # Update completed objectives tracking
        new_completed_objectives = self._update_completed_objectives(player_id, objective)
        
        # Award victory points
        new_victory_points = self._update_victory_points(player_id, objective)
        
        # Update phase-specific scoring tracking
        new_status_phase_scoring = self._update_status_phase_scoring(player_id, objective, current_phase)
        
        # Remove secret objective from player's hand when scored
        new_player_secret_objectives = self._update_secret_objectives_after_scoring(player_id, objective)
    ...
    def _update_completed_objectives(self, player_id: str, objective: "Objective") -> dict[str, list[str]]:
    ...
    def _update_secret_objectives_after_scoring(self, player_id: str, objective: "Objective") -> dict[str, list["Objective"]]:
        ...
        if not objective.is_public and player_id in new_player_secret_objectives:
            new_player_secret_objectives[player_id] = [
                obj for obj in new_player_secret_objectives[player_id] 
    ...
    def can_player_see_objective(self, player_id: str, objective: "Objective") -> bool:
        ...
        for pid, completed in self.completed_objectives.items():
    ...
    def execute_imperial_primary_ability(self, player_id: str) -> "GameState":
        ...
        player_secrets = self.get_player_secret_objectives(player_id)
        if len(player_secrets) >= 3:
            raise ValueError(f"Cannot draw secret objective - player {player_id} already at maximum of 3")
    ...
    def execute_imperial_secondary_ability(self, player_id: str) -> "GameState":
        ...
        if player_influence < 1:
            raise ValueError(f"Insufficient influence - requires 1 influence, player {player_id} has {player_influence}")
    ...
    def spend_player_influence(self, player_id: str, amount: int) -> "GameState":
        ...
        if current_influence < amount:
            raise ValueError(f"Insufficient influence - requires {amount}, player {player_id} has {current_influence}")
