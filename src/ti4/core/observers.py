...
from .events import GameEvent, GameEventBus
...
class EventObserver(ABC):
    ...
    def _extract_event_type_identifier(self, event: GameEvent) -> str:
...
class LoggingObserver(EventObserver):
    ...
    def _log_event_by_type(self, event: GameEvent, event_type: str) -> None:
    ...
    def _log_unit_moved_event(self, event: GameEvent) -> None:
        ...
        """
        logger.info(
            f"Unit moved: {event.unit_id} from {event.from_system} to {event.to_system} by {event.player_id}"
        )

    def _log_phase_changed_event(self, event: GameEvent) -> None:
        ...
        """
        logger.info(
            f"Phase changed: {event.from_phase} -> {event.to_phase} (Round {event.round_number})"
        )

    def _log_combat_started_event(self, event: GameEvent) -> None:
        ...
        """
        logger.info(
            f"Combat started in {event.system_id} with participants: {event.participants}"
        )

    def _log_generic_event(self, event: GameEvent, event_type: str) -> None:
...
class AITrainingDataCollector(EventObserver):
        ...
        training_record.update(
            {
                ...
                "from_system": event.from_system,
                "to_system": event.to_system,
                "player_id": event.player_id,
            }
        )
    ...
    def _add_combat_started_data(
        ...
        """
        training_record.update(
            {"system_id": event.system_id, "participants": event.participants}
        )
