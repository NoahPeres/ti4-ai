...
class StrategyCardSelectionDecision(PlayerDecision):
    ...
    def execute(self, state: Any, player_id: Any) -> ActionResult:
        ...
        return ActionResult(
            ...
            message=result.error_message if not result.success else f"Selected {self.card_type.value}"
        )
...
class StrategyCardActivationDecision(PlayerDecision):
    ...
    def execute(self, state: Any, player_id: Any) -> ActionResult:
        ...
        result = manager.activate_strategy_card_via_coordinator(player_id, self.card_type)

        return ActionResult(
            success=result.success,
            new_state=state,  # State is modified in place
            message=result.error_message if not result.success else f"Activated {self.card_type.value}"
        )
...
class SecondaryAbilityDecision(PlayerDecision):
    ...
    def is_legal(self, state: Any, player_id: Any) -> bool:
        ...
        coordinator = state.strategy_card_coordinator
        return bool(coordinator.can_use_secondary_ability(str(player_id), self.card_type))
    ...
    def execute(self, state: Any, player_id: Any) -> ActionResult:
        ...
        return ActionResult(
            success=success,
            new_state=state,  # State is modified in place
            message=f"Used secondary ability of {self.card_type.value}" if success else "Failed to use secondary ability"
        )
